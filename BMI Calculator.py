...
def BMI():
    ...
    if bmi<=18.5:
        ...
        label3.config(text="You are lower than Normal Body!")

    elif bmi>18.5 and bmi<=25:
        label2.config(text="Normal")
        label3.config(text="You are Healthy Body!")

    elif bmi>25 and bmi<=30:
        label2.config(text="Overweight")
        label3.config(text="Your are slightly over weight, \n you need to lose your weight!")
...
def slider_changed(event):
    ...
    secondimage.place(x=70,y=500-size)
...
slider=ttk.Scale(root,from_=0, to=200,orient="horizontal",style="TScale",command=slider_changed2,
...
Button(root,text="View Report",width=15,height=2,font="arial 10",bg="#1f6e68",fg="white",command=BMI).place(x=280,y=340)
